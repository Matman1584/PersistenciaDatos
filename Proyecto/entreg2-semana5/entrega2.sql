...
--          INSERTS
-- Lleando PAISES
INSERT INTO PAISES (PAISE_ID, pais_nombre) VALUES
(1, 'Colombia'),
(2, 'Argentina'),
(3, 'México'),
(4, 'España'),
...
-- Llenando CIUDADES (eL '1' Al final hacer referencia a Colombia por su ID)
...
-- Llenando LOCALIZACIONES (Asignándolas a Bogotá y Pereira, IDs 1 y 3)
...
-- 4. Llenando DEPARTAMENTOS
-- Usamos los IDs de Localizaciones creados antes: 1(Fontibón), 2(Hayuelos), 3(Plaza Claro), 4(Pereira)
...
-- 5. Llenando EMPLEADOS
-- Orden: ID, Nombre1, Nombre2, Email, Fecha Nacimiento, Sueldo, Comisión, Cargo_ID, Dpto_ID, Gerente_ID
-- NOTA IMPORTANTE: El empleado 1 tiene 'NULL' en el ID del Gerente porque es el jefe de la empresa y no le reporta a nadie. Los demás empleados tienen un número al final que indica quién es su jefe.
...
-- 6. Llenando HISTORICO (Registro de cuando alguien renuncia o cambia de cargo)
-- Orden: ID del registro, Fecha de retiro, Cargo_ID, Dpto_ID, Empleado_ID
